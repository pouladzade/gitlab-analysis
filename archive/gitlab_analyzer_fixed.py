...
class GitLabAnalyzer:
        def __init__(self, gitlab_url: str = None, private_token: str = None, local_only: bool = False):        # Use the provided URL or default to PTC Telematik GitLab        self.ssh_url = gitlab_url or 'git@gitlab.ptc-telematik.de'        # For API access, we need the HTTPS URL        self.api_url = 'https://gitlab.ptc-telematik.de'        self.ssh_key_path = os.path.expanduser('~/.ssh/id_ed25519')        self.local_only = local_only                if not local_only:            if not os.path.exists(self.ssh_key_path):                raise FileNotFoundError(f"SSH key not found at {self.ssh_key_path}")                            try:                # Initialize GitLab API client with HTTPS URL                self.gl = gitlab.Gitlab(self.api_url, private_token=private_token)                # Only authenticate, no write operations                self.gl.auth()                print(f"Successfully connected to GitLab instance: {self.api_url}")            except Exception as e:                print(f"Error connecting to GitLab instance {self.api_url}: {str(e)}")                print("Please ensure:")                print("1. The GitLab URL is correct")                print("2. You have network access to the GitLab instance")                print("3. Your SSH key is properly configured")                print("4. You have a valid personal access token with read_api scope")                raise        else:            print("Running in local analysis mode (no GitLab connection required)")                    self.projects_dir = os.path.join(os.getcwd(), 'projects')        os.makedirs(self.projects_dir, exist_ok=True)
    ...
    def save_analysis_report(self, df: pd.DataFrame, args) -> str:
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            ...
            f.write("DETAILED REPORT BY AUTHOR (CONSOLIDATED BY EMAIL):\n")
            f.write("-" * 100 + "\n")
            ...
            for author_email, row in df.iterrows():
                ...
                f.write(f"{author_display:<50} | {row['added']:>10} | {row['removed']:>10} | {row['net']:>10}\n")
                
            f.write("\n" + "=" * 100 + "\n")
            f.write("Report generated by GitLab Analyzer (Email-Consolidated Version)\n")
    ...
    def save_analysis_report_csv(self, df: pd.DataFrame, args) -> str:
        ...
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        csv_file = os.path.join(self.projects_dir, f'ptc_telematik_analysis_report_consolidated_{timestamp}.csv')
        
        # Add metadata as a comment at the top
        with open(csv_file, 'w', encoding='utf-8') as f:
            f.write(f"# PTC Telematik GitLab Analysis Report (Email-Consolidated)\n")
            f.write(f"# Generated on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"# Analysis Period: {args.since} to {args.until}\n")
            if args.authors:
                f.write(f"# Filtered Authors: {', '.join(args.authors)}\n")
            ...
            f.write(f"# Note: Authors grouped by email address to consolidate duplicate identities\n")
            f.write(f"#\n")
        
        # Append the DataFrame
        df.to_csv(csv_file, mode='a', index_label='Email')
        
        print(f"Analysis report (CSV) saved to: {csv_file}")
        return csv_file

def parse_args():
    parser = argparse.ArgumentParser(description='Analyze GitLab repository contributions with email-based consolidation')
    ...
    parser.add_argument('--apps-only', action='store_true', help='Analyze only Apps group repositories')
    parser.add_argument('--list-repos', action='store_true', help='Only list active repositories without analysis')
    parser.add_argument('--analyze-local', action='store_true', help='Analyze already cloned repositories offline')
    parser.add_argument('--clone-and-analyze', action='store_true', help='Clone repositories and analyze them')
    return parser.parse_args()

def main():
    print("PTC Telematik GitLab Repository Analyzer (Email-Consolidated Version)")
    print("=" * 80)
    
    args = parse_args()
    
        # Get private token from environment    private_token = os.getenv('GITLAB_PRIVATE_TOKEN')        # For local analysis only, we don't need the GitLab token    if args.analyze_local:        if not private_token:            print("Running in local analysis mode (no GitLab token required)")        try:            # Create analyzer without connecting to GitLab for local analysis            analyzer = GitLabAnalyzer(private_token=private_token) if private_token else None        except:            # If connection fails, create a minimal analyzer for local work            analyzer = type('LocalAnalyzer', (), {})()            analyzer.projects_dir = os.path.join(os.getcwd(), 'projects')            analyzer.analyze_local_repositories = GitLabAnalyzer.analyze_local_repositories.__get__(analyzer)            analyzer.analyze_repository = GitLabAnalyzer.analyze_repository.__get__(analyzer)            analyzer.is_merge_commit = GitLabAnalyzer.is_merge_commit.__get__(analyzer)            analyzer.save_analysis_report = GitLabAnalyzer.save_analysis_report.__get__(analyzer)            analyzer.save_analysis_report_csv = GitLabAnalyzer.save_analysis_report_csv.__get__(analyzer)    else:        if not private_token:            print("Error: GITLAB_PRIVATE_TOKEN environment variable not set")            print("Please set your GitLab private token in the .env file or environment")            return        try:            analyzer = GitLabAnalyzer(private_token=private_token)
        
        if args.list_repos:
            # Just list active repositories
            if args.apps_only:
            ...
            if repos:
                analyzer.save_repository_list(repos)
            
        elif args.analyze_local:
            # Analyze already cloned repositories
            print("Analyzing local repositories...")
            ...
            if not repo_paths:
                ...
                print("Please run with --clone-and-analyze first to clone repositories")
                return
            ...
            if not df.empty:
                # Save reports
                analyzer.save_analysis_report(df, args)
                analyzer.save_analysis_report_csv(df, args)
            else:
                print("No analysis data generated.")
        
        elif args.clone_and_analyze:
            # Clone repositories and analyze
            if args.apps_only:
                repos = analyzer.get_apps_repositories(args.since)
            else:
                repos = analyzer.get_active_repositories(args.since)
            ...
            if not repos:
                print("No active repositories found for the specified criteria.")
                return
            ...
            print("  --list-repos: List active repositories")
            print("  --analyze-local: Analyze already cloned repositories")
            print("  --clone-and-analyze: Clone and analyze repositories")
